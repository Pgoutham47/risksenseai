...
import { LayoutDashboard, Building2, Bell, BarChart3, Settings, Shield, Search, User, ChevronLeft, ChevronRight } from 'lucide-react';
    ...
    <div className="flex h-screen w-full overflow-hidden bg-background">
      ...
      <aside className={`flex flex-col border-r border-border bg-sidebar transition-all duration-200 ${collapsed ? 'w-16' : 'w-56'}`}>
        ...
        <div className="border-t border-border px-3 py-3 space-y-2">
          ...
          <button onClick={() => setCollapsed(!collapsed)} className="w-full flex items-center justify-center py-1 text-muted-foreground hover:text-foreground transition-colors">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <header className="h-14 border-b border-border flex items-center justify-between px-6 bg-card flex-shrink-0">
          <h1 className="font-heading text-lg tracking-wider text-foreground">{pageTitle}</h1>
            ...
            <div className="relative">
              {searchOpen ? (
                <input
                  ...
                  className="bg-secondary border border-border rounded-md px-3 py-1.5 text-sm text-foreground w-56 focus:outline-none focus:ring-1 focus:ring-primary"
                />
              ) : (
