  ...
  return (
    <PageTransition>
      <div className="space-y-6">
          ...
          {[
            ...
            <div key={i} className="kpi-card flex items-start justify-between">
              <div>
                ...
                {kpi.isString ? (
                  <p className="font-mono text-2xl text-foreground">{kpi.value}</p>
                ) : (
                  <p className={`font-mono text-2xl ${kpi.danger ? 'text-destructive' : 'text-foreground'}`}>
                    <AnimatedScore value={kpi.value as number} />
                  </p>
                )}
              </div>
              <div className="p-2 rounded-md bg-secondary">{kpi.icon}</div>
            </div>
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="space-y-6">
            ...
            <div className="panel p-5">
              <h3 className="font-heading text-sm tracking-wider text-muted-foreground mb-4">Risk Distribution</h3>
                ...
                <div className="space-y-2">
                  {bandDistribution.map(b => (
                    <div key={b.name} className="flex items-center gap-2">
                      ...
                      <span className="font-mono text-xs text-foreground ml-auto">{b.value}</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {/* Top 5 At-Risk */}
            <div className="panel p-5">
              <h3 className="font-heading text-sm tracking-wider text-muted-foreground mb-4">Top 5 At-Risk Agencies</h3>
                ...
                <tbody>
                  {topAtRisk.map(a => (
                    <tr key={a.id} className="border-b border-border/50 hover:bg-secondary/50 cursor-pointer transition-colors" onClick={() => navigate(`/agency/${a.id}`)}>
                      ...
                      <td className="text-center font-mono text-foreground">{a.trustScore}</td>
          ...
          <div className="space-y-6">
            {/* Live Event Feed */}
            <div className="panel p-5">
              <h3 className="font-heading text-sm tracking-wider text-muted-foreground mb-4">Live Event Feed</h3>
            ...
            {/* Heatmap */}
            <div className="panel p-5">
              <h3 className="font-heading text-sm tracking-wider text-muted-foreground mb-4">Signal Activity Heatmap</h3>
                        ...
                        {days.map((_, di) => {
                          ...
                          return (
                            <td key={di} className="p-0.5">
                              <div className="w-full h-6 rounded-sm" style={{ background: `hsl(38, 90%, 55%, ${opacity * 0.8 + 0.05})` }} title={`${val} agencies`} />
                            </td>
                          );
                        })}
                      </tr>
        ...
        <div className="panel p-5">
          <h3 className="font-heading text-sm tracking-wider text-muted-foreground mb-4">Score Movement Timeline — 30 Days</h3>
          <ResponsiveContainer width="100%" height={220}>
            <LineChart data={scoreHistory}>
              ...
              <YAxis domain={[30, 80]} tick={{ fill: 'hsl(215, 15%, 55%)', fontSize: 10 }} axisLine={{ stroke: 'hsl(220, 12%, 18%)' }} />
              <Tooltip contentStyle={{ background: 'hsl(220, 14%, 11%)', border: '1px solid hsl(220, 12%, 18%)', borderRadius: 6, fontSize: 12, color: 'hsl(210, 20%, 95%)' }} />
              <Line type="monotone" dataKey="avgScore" stroke="hsl(38, 90%, 55%)" strokeWidth={2} dot={false} />
              {scoreHistory.filter(d => d.event).map((d, i) => (
                <ReferenceLine key={i} x={d.date} stroke="hsl(0, 65%, 55%)" strokeDasharray="4 4" label={{ value: d.event, position: 'top', fill: 'hsl(215, 15%, 55%)', fontSize: 9 }} />
              ))}
            </LineChart>
          </ResponsiveContainer>
        </div>
