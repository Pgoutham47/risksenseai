      ...
      <div className="space-y-6 max-w-4xl">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {(Object.keys(thresholds) as (keyof typeof thresholds)[]).map(band => (
              <div key={band} className="space-y-1.5">
                ...
                <input type="number" value={thresholds[band]} onChange={e => setThresholds({ ...thresholds, [band]: Number(e.target.value) })} className="w-full bg-background border border-border rounded-lg px-3 py-2 text-sm font-mono text-foreground focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary/40" />
              </div>
            ))}
          </div>
        ...
        <div className="panel p-6">
          ...
          <div className="space-y-2 mb-5">
            {Object.keys(alertToggles).map(type => (
              <div key={type} className="flex items-center justify-between py-1.5">
                ...
                <button onClick={() => setAlertToggles({ ...alertToggles, [type]: !alertToggles[type] })} className={`w-10 h-[22px] rounded-full transition-colors relative ${alertToggles[type] ? 'bg-primary' : 'bg-secondary'}`}>
          ...
          <div>
            ...
            <input value={escalationEmail} onChange={e => setEscalationEmail(e.target.value)} className="w-full max-w-sm bg-background border border-border rounded-lg px-3 py-2 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary/40" />
          </div>
        </div>
        ...
        <div className="panel p-6">
          ...
          <div className="flex items-center gap-3">
            <input type="number" value={tenurePeriod} onChange={e => setTenurePeriod(Number(e.target.value))} className="w-24 bg-background border border-border rounded-lg px-3 py-2 text-sm font-mono text-foreground focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary/40" />
        ...
        {/* Save */}
        <div className="flex items-center gap-3">
          <button onClick={handleSave} className="bg-primary text-primary-foreground px-6 py-2.5 rounded-lg text-sm font-medium hover:bg-primary/90 transition-colors shadow-sm">Save Configuration</button>
