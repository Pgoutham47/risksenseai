      ...
      <div className="space-y-5">
        ...
        <div className="flex flex-wrap gap-3 items-center">
          <div className="relative flex-1 max-w-sm">
            ...
            <input value={search} onChange={e => { setSearch(e.target.value); setPage(1); }} placeholder="Search by name or ID..." className="w-full bg-card border border-border rounded-lg pl-9 pr-3 py-2 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary/40" />
          </div>
          <select value={bandFilter} onChange={e => { setBandFilter(e.target.value as any); setPage(1); }} className="bg-card border border-border rounded-lg px-3 py-2 text-sm text-foreground appearance-none cursor-pointer focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary/40">
            <option value="ALL">All Bands</option>
            {(['CLEAR', 'CAUTION', 'WARNING', 'RESTRICTED', 'BLOCKED'] as Band[]).map(b => <option key={b} value={b}>{b}</option>)}
          </select>
          <select value={tenureFilter} onChange={e => { setTenureFilter(e.target.value as any); setPage(1); }} className="bg-card border border-border rounded-lg px-3 py-2 text-sm text-foreground appearance-none cursor-pointer focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary/40">
                  ...
                  <td className="py-3 px-4">
                    <div className="flex items-center gap-2">
                      <div className="flex-1 h-1.5 bg-secondary rounded-full overflow-hidden">
                        <div className="h-full rounded-full" style={{ width: `${a.utilization}%`, background: a.utilization > 70 ? 'hsl(0, 72%, 51%)' : a.utilization > 50 ? 'hsl(38, 92%, 50%)' : 'hsl(152, 60%, 36%)' }} />
                      </div>
          ...
          <div className="flex items-center justify-between text-xs text-muted-foreground">
            ...
            <div className="flex gap-1">
              {Array.from({ length: totalPages }, (_, i) => (
                <button key={i} onClick={() => setPage(i + 1)} className={`w-7 h-7 rounded-lg flex items-center justify-center text-xs font-medium ${page === i + 1 ? 'bg-primary text-primary-foreground' : 'bg-secondary text-muted-foreground hover:text-foreground'}`}>{i + 1}</button>
              ))}
            </div>
          </div>
        )}
